...
import { resolvePath } from '@workspacecord/core';
import type { ThreadSession, ProviderName } from '@workspacecord/core';
import * as registry from '@workspacecord/engine/session-registry';
...
export async function registerLocalSession(
  ...
  const { getProjectByPath, getAllRegisteredProjects } = await import('@workspacecord/engine/project-registry');
